...
if st.session_state['page'] == 'main':
    ...
    if st.button("Get Recommendation"):
        ...
        # Save user inputs in session state
        st.session_state['genres'] = genres
        st.session_state['streaming_services'] = streaming_services
    ...
    if st.button("Back"):
        ...
        streaming_services = st.session_state['streaming_services']
        ...
        streaming platform is one of the ones I mentioned in the beginning of this request. If I didn't mention it, please don't recommend something from it. 
        ...
        # Display each recommendation vertically for better mobile experience
        for result in parsed_results:
            ...
            st.subheader(title)
            if cover_data:
                st.image(cover_data, width=200)
            else:
                st.text("Cover art not found")
            if rotten_score:
                st.write(f"**{service}** - IMDB Score: {rotten_score}")
            else:
                st.write(f"**{service}** - IMDB Score: Not available")
        ...
        if st.button("Back to Main Page", key='back_bottom'):
            st.session_state['page'] = 'main'
